...
struct Monkey {
    name: String,
    items: Vec<i32>,
    increase_worry_level: fn(&i32) -> i32,
    inspect_item: fn(&i32) -> usize,
}
...
fn main() { 
    ...
    let mut inspection_counts = vec![0,0,0,0,0,0,0,0];
    ...
    let monkey_business_level = inspection_counts.iter().rev().take(2).product::<i32>();
