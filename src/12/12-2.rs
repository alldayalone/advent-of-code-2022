...
fn main() { 
    ...
    let min = (0..n).flat_map(|i| {
        let brah = heightmap[i].clone();
        (0..m).filter_map(move |j| {
            // Some(i)
            if brah[j] == 'a' { Some((i, j)) } else { None }
        })
    }).filter_map(|start_point| {
